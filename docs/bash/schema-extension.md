...
```bash
# 当前目录有 a.txt 文件
# 如果?.txt可以扩展成文件名，echo命令会输出扩展后的结果
$ echo ?.txt
a.txt

# 当前目录为空目录
# 如果不能扩展成文件名，echo就会原样输出?.txt
$ echo ?.txt
?.txt
```
...
::: warning 注意
`*`不会匹配隐藏文件（以`.`开头的文件），即`ls *`不会输出隐藏文件。
:::
...
```bash
# 存在 aaa、bbb、aba 三个文件
# [!a]表示文件名第二个字符不是a的文件名，所以返回了aba和bbb两个文件
$ ls ?[!a]?
aba bbb
```
